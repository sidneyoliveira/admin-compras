...
async function gerarArq(value){
  ...
  for (var i = 0;i < ArrayIDs.length; i++){
    ...
    const data_user = await fetch("usuarios.php?id=" + id_user);
    var users = await data_user.json();

    var usuario = users.users[0][1];
    var cargo = users.users[0][2];
    var cpf = users.users[0][3];
  ...
  //             callback(ip);
  //         } else {
  //             callback("errrroo"); // Em caso de erro, retorne null
  //         }
  //     };
  //     xhr.send();
  // }
  ...
  // }); 
    ...
    <table id="assi">
      <thead>
        <tr scope="col" style="height: 5em; border: none !important;border-collapse:collapse;">
          <td>
          <p style="font-size: 0.85em; margin: 5px !important">${usuario}</p>
          <p style="font-size: 0.7em; margin: 5px !important">${cargo}</p>
          <p style="font-size: 0.5em; margin: 0px !important">Assinado eletronicamente por: ${usuario} - CPF: ${cpf}</p>
          <p style="font-size: 0.5em; margin: 0px !important">em ${data} - 09:00 </p>
        </td>
        </tr>
      </thead>
    </table>
...
// Seu HTML que deseja converter em PDF
    setTimeout(function() { 
    const htmlElement = document.getElementById('frame1');
    
    // GrabzIt("ZGQxY2U0NDZmOGFlNGQ3Y2I5MDk0MTc4ZGYyODUyZjQ=").ConvertHTML(htmlContent, 
    // {"format": "pdf", "download": 1}).Create();

    console.log(htmlElement);
    // Use a função fromHTML para converter o HTML em PDF
    htmlElement.contentWindow.print();
